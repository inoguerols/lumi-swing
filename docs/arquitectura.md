...
|---|---|---|
| **Dispositivos** | **iPhone + iPad + Mac (Apple Silicon, «Designed for iPad»)** | v1.1 universal (D-023, `decisiones.md`): sin target nuevo, checkbox de ASC. El fallback sin Taptic Engine (`lenguaje-haptico.md` §6.1) ya cubría la sensorial en iPad/Mac. |
| **Orientación** | **Portrait, bloqueado** (D-003, modulada por D-023) | Un solo pulgar, sesiones de 15–60 s, uso en el metro. El mundo avanza en horizontal, así que portrait recorta anticipación visual: se compensa con cámara adelantada (`lookAhead`) y velocidad moderada. Y en las zonas a ciegas ese coste es cero por definición — el juego está diseñado para no depender de la vista en su mecánica firma. Landscape daría más campo pero exige dos manos y rompe el gesto de "una mano, una vez más". D-023 amplía la familia de dispositivos sin tocar esta regla. |
| **Tamaño de escena** | **Altura fija 1950 / ancho variable [900, 1462]** (D-023) | `Tuning.World.sceneSize(for:)` calcula ancho = 1950 × aspecto de la vista, clampeado a ese rango (1462 = iPad 4:3 exacto). Misma franja vertical de juego en todos los dispositivos; el ancho extra en iPad lo cubre el bleed del arte ya existente. Antes era 900×1950 fijo con `.aspectFill`, que recortaba suelo/techo en iPad. |
